  ...
  return (
    <main className="w-full mb-20 lg:pt-[119px]" style={{ backgroundImage: 'linear-gradient(180deg, #1E1E1E 12.88%, rgba(0, 0, 0, 0) 100%)', backgroundSize: '100% 860px', backgroundRepeat: 'no-repeat' }}>
      ...
      <div className="w-full lg:mb-[60px] text-center ">
        <h1 className="text-2xl md:text-[50px] font-monument mb-3.5">About <span className="text-accent">Us</span></h1>
        <p className="font-medium lg:text-lg">Any question or remarks? React out to us!</p>
      </div>
      {/* WHO WE ARE */}
      <div className="w-full mt-20 pb-5 relative lg:mb-[73px]">
        ...
        <div className="flex md:w-[90%] w-[85%] mx-auto">
          <svg className="w-4 h-40 mr-1.5 md:w-8 md:h-72 lg:hidden" viewBox="0 0 30 304" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* CARD */}
      <div className="mx-auto w-[90%] mb-[61px] rounded-t-[20px] overflow-hidden" style={{ background: 'linear-gradient(180deg, rgba(255, 255, 255, 0.1) 0%, rgba(255, 255, 255, 0) 100%)' }}>
        <div className="relative w-full h-[259px] mb-5 ">
          <Image src={"/aboutus.png"} fill alt="about" className="object-cover" style={{ objectFit: "cover" }} />
          {/* OVERLAY OVER IMAGE */}
          <div class="absolute top-0 left-0 right-0 bottom-0"
            style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.8) 2.67%, rgba(0,0,0,0.48)) 100%' }}>
          </div>
        </div>
        <div className="w-full h-[208px] flex justify-between px-8">
          <div className="w-[414px] ">
            <h2 className="font-monument lg:text-[36px] text-center mb-6">Our Vision</h2>
            <p className="font-medium lg:text-sm text-justify">To be a leading digital agency, empowering businesses with innovative solutions that transcend the digital realm. We envision a future where our strategic insights, creative brilliance, and technological expertise converge to drive unparalleled success for our clients.</p>
          </div>
          <div className="lg:w-[717px]">
            <h2 className="font-monument lg:text-[36px] text-center mb-6">Our Mission</h2>
            <div className="flex justify-center gap-11 relative">
              {data.map((item) => (
                <div className="flex items-center bg-white-20 w-[203px] px-[18px] py-6 gap-3 rounded-lg" key={item.id}>
                  <div style={{ width: "28px", height: "28px", background: '#FFF2ED', flex: "none" }}></div>
                  <p className="font-medium lg:text-sm">{item.text}</p>
                </div>
              ))}
            </div>
          </div>
